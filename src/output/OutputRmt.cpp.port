...
static S3RmtChannel s3_rmt_channels[8];

static esp_err_t s3_rmt_init_channel(int chan, gpio_num_t gpio, size_t mem_blocks, uint32_t clk_divider) {
    rmt_config_t cfg;
    cfg.rmt_mode = RMT_MODE_TX;
    cfg.channel = (rmt_channel_t)chan;
    cfg.gpio_num = gpio;
    cfg.mem_block_num = mem_blocks;
    cfg.clk_div = clk_divider;
#if defined(RMT_TX_CARRIER_EN)
    ...
    if (err != ESP_OK) return err;
    s3_rmt_channels[chan].channel = cfg.channel;
    s3_rmt_channels[chan].gpio = cfg.gpio_num;
    s3_rmt_channels[chan].mem_block_num = cfg.mem_block_num;
    s3_rmt_channels[chan].tx_done_sem = xSemaphoreCreateBinary();
    s3_rmt_channels[chan].tx_limit = 0;
    return ESP_OK;
}

static esp_err_t s3_rmt_deinit_channel(int chan) {
    rmt_channel_t channel = (rmt_channel_t)chan;
    if (s3_rmt_channels[chan].tx_done_sem) {
        vSemaphoreDelete(s3_rmt_channels[chan].tx_done_sem);
        s3_rmt_channels[chan].tx_done_sem = NULL;
    }
    return rmt_driver_uninstall(channel);
}

static esp_err_t s3_rmt_write_items_blocking(int chan, rmt_item32_t* items, size_t num_items) {
    rmt_channel_t channel = (rmt_channel_t)chan;
    esp_err_t err = rmt_write_items(channel, items, num_items, true);
    // ensure transmission finished
    rmt_wait_tx_done(channel, portMAX_DELAY);
    return err;
}

static void s3_rmt_start_tx(int chan) {
    // No direct register to start; RMT starts when items are written via rmt_write_items
    (void)chan;
}

static void s3_rmt_stop_tx(int chan) {
    // Stop transmitter by disabling driver (or wait for done). Keep it simple:
    rmt_channel_t channel = (rmt_channel_t)chan;
    rmt_tx_stop(channel);
}

static void s3_rmt_reset_mem_ptrs(int chan) {
    // Not applicable; managed by driver. Provide a nop for compatibility.
    (void)chan;
}

static void s3_rmt_clear_interrupts(int chan) {
    // Interrupts are managed by driver; nop for now.
    (void)chan;
}

static void s3_rmt_set_limit(int chan, int limit) {
    s3_rmt_channels[chan].tx_limit = limit;
}
#endif
...
c_OutputRmt::~c_OutputRmt ()
{
    ...
    if (HasBeenInitialized)
    {
        ...
        #else
        // ESP32-S3 / other targets: RMT register access skipped here.
        // TODO: replace this block with the new RMT driver API (rmt_tx_channel_* functions)
        #endif
    }
